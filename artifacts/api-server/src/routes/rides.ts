...
// Rota real via OpenRouteService (ORS) — fallback para haversine
async function getRouteData(originLat: number, originLng: number, destLat: number, destLng: number): Promise<{ distanceKm: number; durationMin: number }> {
  try {
    ...
    // Usando OpenRouteService em vez de OSRM para maior confiabilidade
    const url = `https://api.openrouteservice.org/v2/directions/driving-car?api_key=${apiKey}&start=${originLng},${originLat}&end=${destLng},${destLat}`;
    ...
    console.error("Erro ao buscar rota no backend (ORS):", error);
  }
  
  // Fallback Haversine se a API falhar
  const R = 6371;
...
router.patch("/rides/:id/status", requireAuth, async (req, res): Promise<void> => {
  ...
  if (status === "cancelled") {
    ...
    emitToRide(id, "ride_status_update", { rideId: id, status: "cancelled", reason });
  }

  await db.update(ridesTable).set(updates).where(eq(ridesTable.id, id));
